# This is the code for the start of Karpathy's blog post on GPT up to the first introduction of transformer heads
...
class Head(nn.Module):
    def __init__(self, input_channels, size=16, block_size=8) -> None:
        super().__init__()
        self.size = size
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self, embedding_size, vocab_size=vocab_size, block_size=block_size):
        super().__init__()
        # each token directly reads off the logits for the next token from a lookup table
        self.token_embedding_table = nn.Embedding(vocab_size, embedding_size)
        self.position_embedding_table = nn.Embedding(block_size, embedding_size)
        self.head = Head(embedding_size, size=embedding_size, block_size=block_size)
